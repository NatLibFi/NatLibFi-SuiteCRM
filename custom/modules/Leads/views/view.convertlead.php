...
class CustomViewConvertLead extends ViewConvertLead
{
    ...
    public function display()
    {
        ...
        $contactData = $this->getContactData($this->focus->id);
        if (!empty($contactData))
        {
            $contact = new Contact();
            $contact->retrieve($contactData[0]['id']);
            $filter .= '&first_name_advanced=' . urlencode($contact->first_name);
            $filter .= '&last_name_advanced=' . urlencode($contact->last_name);
        } else {
        ...
        foreach($this->defs as $module => $vdef)
        {
            ...
            foreach($focus->field_defs as $field => $def)
            {
            	if(isset($vdef[$ev->view]['copyData']) && $vdef[$ev->view]['copyData'])
                {
	                ...
	                elseif ($module === 'Accounts' && $field === 'description')
	                {
	                ...
	                elseif ($module === 'Accounts' && $field === 'industry')
	                {
	                    $focus->industry = $this->focus->sektori_c;
	                }
	                elseif ($module === 'Accounts' && $field === 'phone_office')
	                {
	                    $focus->phone_office = $this->focus->phone_office_c;
	                }
	                elseif ($module === 'Accounts' && $field === 'phone_alternate')
	                {
