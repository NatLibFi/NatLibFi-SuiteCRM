...
foreach (array_keys($_POST) as $field) {
    if (substr($field, 0, strlen('Contactsaccount_id')) === 'Contactsaccount_id') {
        $index = substr($field, strlen('Contactsaccount_id'));
    }
    ...
    if (!isset($_POST['Contactsaccount_id' . $index]) || !isset($_POST['Contactsaccount_name' . $index])) {
        continue;
    }

    $id = $_POST['Contactsaccount_id' . $index];
    $name = $_POST['Contactsaccount_name' . $index];
    $department = '';
    $address = '';
    $description = '';
    ...
    if (isset($_POST['Contactsaccount_department' . $index])) {
        $department = $_POST['Contactsaccount_department' . $index];
    }
    if (isset($_POST['Contactsaccount_address' . $index])) {
        $address = $_POST['Contactsaccount_address' . $index];
    }
    if (isset($_POST['Contactsaccount_description' . $index])) {
        $description = $_POST['Contactsaccount_description' . $index];
    }

    $input .= "<input type='hidden' name='account_id$index' value='$id'>\n";
    $input .= "<input type='hidden' name='account_name$index' value='$name'>\n";
    $input .= "<input type='hidden' name='account_department$index' value='$department'>\n";
    $input .= "<input type='hidden' name='account_address$index' value='$address'>\n";
    $input .= "<input type='hidden' name='account_description$index' value='$description'>\n";
}
