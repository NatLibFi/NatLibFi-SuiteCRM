...
function handleAllianceContactChange() {
    var contactIdField = YAHOO.util.Selector.query('input[name=nlfal_alliances_contacts_1contacts_idb]', null, true);
    ...
    var allianceIdField = YAHOO.util.Selector.query('input[name=return_id]', 'form_SubpanelQuickCreate_nlfal_Alliances_Contacts', true); // TODO: or relate_id?
    var allianceId = allianceIdField.value;
...
if (MutationObserver) {
    var observer = new MutationObserver(function(mutations, observer) {
        if(mutations[0].attributeName == "value") {
            handleAllianceContactChange();
        }
    });
    observer.observe(document.getElementById('nlfal_alliances_contacts_1contacts_idb'), {
       attributes: true
    });
}
