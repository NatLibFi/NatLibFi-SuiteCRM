...
class ContactRoleAwareSearchForm extends SearchForm
{
     ...
     * is simply copied here, and NLF-specific code
     * is added in relevant places.
     * Custom code is marked with NLF-specific comments.
     */
         ...
         foreach((array) $this->searchFields as $field=>$parms) {
			...
			else
			{
				...
				$db_field = $table_name . "." . $field;
                     ...
                     } elseif (isset($parms['subquery_with_multienum_regexp']) && $parms['subquery_with_multienum_regexp']) {
                         // NLF-specific parameter: subquery uses multienum value in the REGEXP match
                         // value will be formated as a regular expression not as a list
                         $field_value = '';
                         foreach ($parms['value'] as $val) {
                             if ($val != ' ' and $val != '') {
                                 if (!empty($field_value)) {
                                 ...
                                 $field_value .= '\^' . $val . '\^'; // TODO: should $val be somehow sanitized?
                             }
                         }
                         if ($field_value !== '') {
