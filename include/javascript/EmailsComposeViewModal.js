...
var url='index.php?module=Emails&action=ComposeView&in_popup=1&targetModule='+targetModule+ids+'&relatedModule='+currentModule+'&relatedId='+relatedId;if(typeof(document.MassUpdate)!=='undefined'&&document.MassUpdate.select_entire_list.value==1){url+='&current_post='+document.MassUpdate.current_query_by_page.value;}
$.ajax({type:"GET",cache:false,url:url}).done(function(data){if(data.length===0){console.error("Unable to display ComposeView");composeBox.setBody(SUGAR.language.translate('','ERR_AJAX_LOAD'));return;}
composeBox.setBody(data);self.emailComposeView=composeBox.controls.modal.body.find('.compose-view').EmailsComposeView();var targetCount=0;var targetList='';var populateModuleName='';var populateEmailAddress='';var populateModule='';var populateModuleRecord='';var recipientName='';var dataEmailName=$(source).attr('data-module-name');var dataEmailAddress=$(source).attr('data-email-address');$('.email-compose-view-to-list').each(function(){if($('.email-relate-target'.length)){populateModule=$('.email-relate-target').attr('data-relate-module');populateModuleRecord=$('.email-relate-target').attr('data-relate-id');populateModuleName=$('.email-relate-target').attr('data-relate-name');}
...
recipientName=$(this).attr('data-record-name');populateEmailAddress=$(this).attr('data-record-email');if(targetCount>0){targetList=targetList+',';}
targetList=targetList+recipientName+' <'+populateEmailAddress+'>';targetCount++;});if(targetCount>0){if(populateEmailAddress!==''){$(self.emailComposeView).find('#to_addrs_names').val(targetList);}
